...
export default function OnboardingOverlay({ visible, onClose }: Props) {
  const slides: Slide[] = useMemo(
    () => [
      ...
      { id: "2", title: "Talk It Out", body: "Start a session with Therapy Buddy. Type or use the mic for voice-to-text." },
      { id: "3", title: "Journals & Notes", body: "Capture insights and save chats into your private journal." },
      { id: "4", title: "Gentle Motivation", body: "Daily quotes to nudge you forward small steps count." },
    ],
    []
  );
  ...
  const goNext = () => {
    if (index < slides.length - 1) animateTo(index + 1, () => setIndex((i) => i + 1));
    else onClose();
  };
